	...
	function formatRow( $row ) {
		...
		if ( $row->notlog_action !== 'removed' ) {
			...
			$htmlOut .= '<a href="javascript:toggleLogDisplay(\''.$notlogId.'\')">'.
				'<img src="'.$wgExtensionAssetsPath.'/CentralNotice/'.$collapsedImg.'" id="cn-collapsed-'.$notlogId.'" style="display:block;"/>'.
				'<img src="'.$wgExtensionAssetsPath.'/CentralNotice/uncollapsed.png" id="cn-uncollapsed-'.$notlogId.'" style="display:none;"/>'.
				'</a>';
		}
		...
		if ( $row->notlog_action !== 'removed' ) {
			...
			$htmlOut .= Xml::openElement( 'tr', array( 'id' => 'cn-log-details-'.$notlogId, 'style' => 'display:none;' ) );
